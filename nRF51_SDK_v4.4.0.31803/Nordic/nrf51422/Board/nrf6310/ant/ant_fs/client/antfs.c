...
#define CHANNEL_NUMBER_MASK                0x1Fu                         /**< ANT channel number bitmask. */
...
#define SEQUENCE_NUMBER_MASK               0xE0u                         /**< ANT burst message sequence number bitmask. */
#define SEQUENCE_NUMBER_ROLLOVER           0x60u                         /**< ANT burst message sequence number rollover bitmask. */
#define SEQUENCE_FIRST_MESSAGE             0x00u                         /**< ANT burst message first message sequence number bitmask. */
#define SEQUENCE_LAST_MESSAGE              0x80u                         /**< ANT burst message last message sequence number bitmask. */
                                   ...
                                   const uint8_t * const p_message)
{
       ...
       )
    {
        ...
        if (m_current_state.sub_state.trans_sub_state == ANTFS_TRANS_SUBSTATE_VERIFY_CRC)
        {
            ...
            uint32_t response = RESPONSE_INVALID_CRC;
            ...
            if (m_link_burst_index.data == m_saved_crc_offset)     
            {
                ...
                if (m_transfer_crc == m_compared_crc)     
                {
                    ...
                    response             = RESPONSE_MESSAGE_OK;
                }
            }
            // If our save point and requested offset are in the same data block, we can calculate 
            // the CRC and compare. 
        ...
        if (m_current_state.sub_state.trans_sub_state == ANTFS_TRANS_SUBSTATE_DOWNLOADING)
        {
            ...
            m_compared_crc       = m_transfer_crc;    
            m_saved_crc_offset   = m_temp_crc_offset;          
            m_temp_crc_offset    = m_link_burst_index.data;
...
void antfs_channel_setup(void)
{
    ...
    err_code = sd_ant_channel_radio_tx_power_set(ANTFS_CHANNEL, ANTFS_TRANSMIT_POWER);
    APP_ERROR_CHECK(err_code);
