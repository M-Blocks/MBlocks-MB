                             ...
                             uint32_t * const        p_buf_len)
{
    ...
    SER_ASSERT((opt_id == BLE_GAP_OPT_CH_MAP) , NRF_ERROR_INVALID_PARAM);
                             ...
                             uint32_t      * const p_result_code)
{
    ...
    SER_ASSERT((*p_opt_id == BLE_GAP_OPT_CH_MAP), NRF_ERROR_INVALID_PARAM);

    switch (*p_opt_id)
    {
      case BLE_GAP_OPT_CH_MAP:
          err_code = ble_gap_opt_ch_map_t_dec( p_buf, packet_len, &index, (void *)&(p_opt->gap.ch_map));
      break;
    }
